...
<body>
	...
	<!-- Restaurant List -->
	<div id = "restListForm">
		<h2 align = center><b>Q&A List</b></h2>
		<table class = "table">
			<thead>
				<tr id = "adapt_center">
					<th scope = "col">Num</th>
			...
			<c:forEach var = "row" items="${pageInfo.list}">
				<tr>
					<th scope = "row" id = "adapt_center">${row.num}</th>
					<td id = "adapt_center">${row.title}</td>
					<td id = "adapt_center">${row.id}</td>
					<td id = "adapt_center">${row.wdate}</td>
					<td>${row.contents}</td>
				</tr>
			</c:forEach>
		</table>
